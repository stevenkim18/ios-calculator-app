...
class DecimalCalculator {
    var opperandStack = Deque<Int>()
    var operatorStack = Deque<String>()
    
    func opperandAndOperatorValidation(inputText: String) {
        if let change = Int(inputText) {
            opperandStack.enque(element: change)
        } else {
            switch inputText {
            case "+", "-", "*", "/":
                operatorStack.enque(element: inputText)
    ...
    func calculation() {
        let inputOperator = operatorStack.deque()
        let opr1 = opperandStack.deque()
        let opr2 = opperandStack.deque()
        
        switch inputOperator {
        case "+":
            let result = opr1! + opr2!
            opperandStack.enque(element: result)
        case "-":
            let result = opr1! - opr2!
            opperandStack.enque(element: result)
        case "*":
...
class BinaryCalculator {
    ...
    func opperandAndOperatorValidation(inputText: String) {
        ...
        if let change = Int(inputText) {
            // 숫자일 경우 숫자스택에 push
            opperandStack.push(element: change)
        } else {
            // 연산자일 경우
            switch inputText {
            case "+", "-", "*", "/":
